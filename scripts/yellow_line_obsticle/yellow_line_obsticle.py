...
class YellowLineObstacle(Node):
    ...
    def pointcloud_callback(self, msg: PointCloud2):
        # Unpack the buffer — our format is 4x float32 per point: x, y, z, rgb
        buf = np.frombuffer(msg.data, dtype=np.float32).reshape(-1, 4)
        if len(buf) == 0:
            return

        # Reinterpret the rgb float32 column as uint32 to extract r, g, b
        rgb_packed = buf[:, 3].view(np.uint32)
        r = (rgb_packed >> 16) & 0xFF
        g = (rgb_packed >> 8)  & 0xFF
        b =  rgb_packed        & 0xFF

        # Yellow in our color map is r=0, g=200, b=200 (BGR packed as RGB)
        # class 1: colors_bgr = (0, 200, 200) → r=0, g=200, b=200
