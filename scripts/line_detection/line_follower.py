...
class BlueLineFollower(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f'Blue line follower started. Waiting for start command on /line_follower/start_command')
    ...
    def _navigate_to_start(self):
        """Navigate to the predefined start pose using Nav2. Return True when reached, False while navigating."""
        ...
        # Check if submission future is done (goal was sent, need to get handle)
        ...
        self.get_logger().info(f'Sending start pose goal to Nav2: ({START_TRANSLATION[0]}, {START_TRANSLATION[1]})')
